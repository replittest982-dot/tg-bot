...
# -*- coding: utf-8 -*-
"""
💎 StatPro v69.3 - TITAN ABSOLUTE POWER (STABLE FIX)
----------------------------------------------------
...
class Worker:
    ...
    def _bind(self):
        ...
        async def quiz_cmd(e):
            ...
            if not question: 
                return await e.edit("❌ Текст не найден!")
        ...
        async def deep_scan(e):
            ...
            try:
                # Фикс: добавлено ignore_migrated=True для стабильности в старых группах
...
async def siphon_file(m: Message, state: FSMContext):
    ...
    ids = []
    try:
        # FIX: Поддержка разных разделителей (для Excel в СНГ)
        ...
        # Безопасное удаление при ошибке
        if os.path.exists(path): os.remove(path)
        return await m.answer(f"❌ Bad CSV: {e}")
    
    # Безопасное удаление
    if os.path.exists(path): os.remove(path)
    
    if not ids: return await m.answer("❌ Empty file (проверь формат CSV!)")
...
async def run_siphon(uid, worker, targets, text):
    ok, fail = 0, 0
    # FIX: Полная защита цикла рассылки
    try:
        for tid in targets:
            try:
        ...
        pass # Глобальная защита от краша таска
