...
def get_numeric_code_keyboard():
    ...
    kb = [
        ...
        [
            ...
            InlineKeyboardButton(text="✅ Ввод", callback_data="auth_submit_code"), # Кнопка для завершения ввода
        ]
    ]
...
async def process_code_input_ui(callback: types.CallbackQuery, state: FSMContext):
    ...
    if action.startswith("auth_digit_"):
        ...
        await callback.message.edit_text("⏳ Проверка кода...", reply_markup=None)
...
# --- Админ-Панель (Полная Реализация Создания Промокода) ---
...
# --- Мониторинг и Отчеты (Остальные хендлеры без изменений) ---
