...
def generate_promocode(length=8) -> str:
    ...
    return ''.join(random.choice(characters) for _in range(length))
...
class AsyncDatabase:
    ...
    async def init(self):
        try:
            # Защита от readonly, используем timeout=20 для устойчивости
            self.db_pool = await aiosqlite.connect(self.db_path, isolation_level=None, timeout=20)
...
class TelethonManager:
    ...
    async def _safe_disconnect(self, client: Optional[TelegramClient]):
        """Безопасно отключает клиент, избегая TypeError и других ошибок на disconnected клиентах."""
        if client:
            try:
                # В Telethon is_connected() - это async метод, поэтому await обязателен!
                if await client.is_connected():
                    await client.disconnect()
            except AttributeError:
                # На случай, если у объекта нет is_connected (очень старая версия или неверный объект)
                try:
                    await client.disconnect()
                except Exception:
                    pass
            except Exception as e:
                # Сюда попадет TypeError: object bool can't be used in 'await' expression
                # Если is_connected() вдруг вернул bool синхронно. Мы игнорируем, но логируем.
                logger.warning(f"Error during safe disconnect (likely TypeError): {type(e).__name__} - {e}")
                try:
                    # Попытка прямого disconnect, если is_connected() сбил с толку
                    await client.disconnect()
                except Exception:
                    pass
    ...
    async def handle_telethon_error(self, user_id: int, error_type: str, e: Exception, message: str):
        ...
        if isinstance(e, sqlite3.OperationalError):
            if 'attempt to write a readonly database' in error_message or 'unable to open database file' in error_message:
                message = "❌ **Критическая ошибка:** База данных/сессия недоступна для записи. **Проверьте права доступа (777)** к папкам `sessions` и `data` на хостинге."
        
        # 2. Обработка AuthKeyUnregisteredError при старте сессии
    ...
    async def start_worker_session(self, user_id: int, client_temp: TelegramClient):
        ...
        try:
            # Убеждаемся, что временный клиент подключен, чтобы получить данные сессии
            ...
            # ЭТОТ ШАГ МОЖЕТ ВЫЗВАТЬ sqlite3.OperationalError: attempt to write a readonly database
            ...
            await self._safe_disconnect(client_perm)
            await self._cleanup_temp_session(user_id)
            return
        ...
        # Разрываем соединение с только что сохраненным постоянным клиентом, чтобы запустить его через start_client_task
        await self._safe_disconnect(client_perm)
    ...
    async def wait_for_qr_scan(self, user_id: int, client: TelegramClient, qr_login: Any, qr_future: asyncio.Future):
        ...
        try:
            # client уже должен быть подключен
...
async def command_login_handler(message: Message, state: FSMContext):
    ...
    # Поскольку логики start_login_process нет в предоставленном коде, используем заглушку.
    # В реальном коде замените это на вызов вашей функции, которая начинает QR-логин
    await manager._send_to_bot_user(
        user_id, 
        "❌ Вставьте сюда логику начала QR-логина (start_login_process) и QR-код. \n"
        "Для продолжения работы бота необходимо реализовать эту функцию!"
    )
...
async def command_status_handler(message: Message):
    ...
    is_active_ram = user_id in store.active_workers
...
async def on_startup(dp):
    ...
    # Убедимся, что папки существуют (повторная проверка)
    os.makedirs(SESSION_DIR, exist_ok=True)
    os.makedirs(DATA_DIR, exist_ok=True)
