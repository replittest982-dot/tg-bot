#!/usr/bin/env python3
"""
🚀 STATPRO ULTIMATE v4.1 - ФИНАЛЬНАЯ ОТЛАДКА ПЕРЕМЕННЫХ
✅ Добавлена принудительная печать переменных BOT_TOKEN, ADMIN_ID, API_ID в лог.
✅ Проверка на отсутствие критических переменных.
✅ Включена заглушка для BaseMiddleware (для обхода проблем с aiogram v3).
"""
...
# Загружаем переменные (для локального запуска или для хостингов, читающих .env)
load_dotenv(override=True)

# ------------------------------------------------------------------------
# 💡 ОТЛАДКА: Проверяем, что переменные действительно загружены
# ------------------------------------------------------------------------
BOT_TOKEN_RAW = os.getenv("BOT_TOKEN", "НЕ НАЙДЕН")
ADMIN_ID_RAW = os.getenv("ADMIN_ID", "НЕ НАЙДЕН")
API_ID_RAW = os.getenv("API_ID", "НЕ НАЙДЕН")

if BOT_TOKEN_RAW != "НЕ НАЙДЕН" and ADMIN_ID_RAW != "НЕ НАЙДЕН":
    print(f"DEBUG_CHECK: BOT_TOKEN загружен. Начало: {BOT_TOKEN_RAW[:10]}")
    print(f"DEBUG_CHECK: ADMIN_ID загружен. Значение: {ADMIN_ID_RAW}")
    print(f"DEBUG_CHECK: API_ID загружен. Значение: {API_ID_RAW}")
else:
    print("DEBUG_CHECK: ОШИБКА! Переменные BOT_TOKEN/ADMIN_ID не найдены!")
# ------------------------------------------------------------------------
